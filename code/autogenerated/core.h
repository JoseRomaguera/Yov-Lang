...
internal_fn void define_core(Interpreter* inter)
{
    ...
    // function call
    {
        ...
        parameters = array_make(p, array_count(p));
        define_intrinsic_function(inter, {}, STR("call"), parameters, VType_Void);
    }
    // function call_exe
    {
        Array<ObjectDefinition> parameters{};
        ...
        parameters = array_make(p, array_count(p));
        define_intrinsic_function(inter, {}, STR("call_exe"), parameters, VType_Void);
    }
    // function exit
    {
        Array<ObjectDefinition> parameters{};
    ...
    // function create_directory
    {
        ...
        ObjectDefinition p[2];
        p[0] = obj_def_make(STR("path"), vtype_from_name(inter, STR("String")));
        p[1] = obj_def_make(STR("recursive"), vtype_from_name(inter, STR("String")));
        parameters = array_make(p, array_count(p));
